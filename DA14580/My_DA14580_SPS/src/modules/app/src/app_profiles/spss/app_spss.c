...
{						
		...
		// Send the message
	 // printf_string(" app_sps_create_db: SPS_SERVER_CREATE_DB_REQ \r\n");
		ke_msg_send(req);
}
/**
 ****************************************************************************************
...
static void app_sps_server_enable(void)
{
    ...
    // Send the message
	  //printf_string(" app_sps_server_enable: SPS_SERVER_ENABLE_REQ \r\n");
    ke_msg_send(req);
}
...
void app_send_ble_flowctrl(uint8_t flowcontrol)
{
    ...
    req->flow_control_state = (uint8_t) flowcontrol;
	  //printf_string(" app_send_ble_flowctrl: SPS_SERVER_SEND_FLOW_CONTROL_REQ \r\n");
    ke_msg_send(req);
}
...
void app_init_ble_tx(void)
{
    struct sps_server_init_ble_tx_req * req = KE_MSG_ALLOC(SPS_SERVER_INIT_BLE_TX_REQ, TASK_SPS_SERVER, TASK_APP,  sps_server_init_ble_tx_req);
	  //printf_string(" app_init_ble_tx: SPS_SERVER_INIT_BLE_TX_REQ \r\n");
    ke_msg_send(req);
}
